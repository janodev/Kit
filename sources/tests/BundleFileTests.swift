...
final class BundleFileTests: XCTestCase
{
    ...
    func testBundleFile_throws_when_reading_a_directory() throws {
        do {
            let filename = "notafile"
            _ = try BundleFile(filename: filename, bundle: Bundle.module).url
        } catch FileError.foundDirectoryButExpectedFile {
            return
        }
        XCTFail("Expected to throw when trying to read a directory")
    }
}
